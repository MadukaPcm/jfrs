    ...
    try:
        if request.method == 'POST':
            ...
            if user is not None and user.is_active:
                ...
                messages.warning(request, 'Blocked || Invalid Credentials!!')
                return redirect('login_url')
...
def UaaUserListView(request):
    try:
        ...
        myRole = request.user.groups.all()[0].name
        vesselBooking = VesselBooking.objects.filter(status=True)
        
        userList = Profile.objects.all()
    except Exception as e:
        return render(None, 'uaa/error500.html')
    
    context = {'roleInstanceData': roleInstance,'userListData':userList,'vesselBooking':vesselBooking}
